// Target path in project: src/components/admin/AdminPartnershipRequestTable.tsx
"use client";
...
export default function AdminPartnershipRequestTable() {
  const [requests, setRequests] = useState<PartnershipRequestRow[]>([]);
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/partnership-requests?${params.toString()}`);
  ...
  async function updateStatus(id: string, status: string) {
    ...
    try {
      const res = await fetch(`/api/partnership-requests/${id}`, {
        method: "PATCH",
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <p className="py-8 text-sm text-slate-500">No partnership requests found.</p>
      ) : (
              ...
              {requests.map((r) => (
                <>
                  <tr key={r._id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-4 py-3 text-slate-700">{r.partnerType}</td>
                    <td className="px-4 py-3">
